  ...
  render() {
    return (
      <div className="card">
        ...
        <p className="delay--5">
          You can see on the left of ROAD_NAME a tavern, with a sign inscribed with <Link to={`/tavern`}>{this.props.tavern.business.name}</Link>.
        </p>
        <p className="delay--6">
          Further down ROAD_NAME is an alchemy lab named <Link to={`/alchemist`}>{this.props.alchemist.business.name}</Link>.
        </p>
      </div>
    );
  }
